...
plt.xticks([1, 2, 3])
plt.grid(True)
print.savefig('Figure01.png')         # 결과를 그림파일로 저장
plt.close()                         # 다음 Plot을 새로 그리기 위해 plt 닫기

"""### **수직 막대 그래프 : 각 승전 항구에 따른 생존자 수 확인하기**"""
...
scatter = plt.scatter(x='Age', y='Fare', data=titanic, c=titanic['Survived'],cmap='Set2',
alpha=0.7)
...
#상관 행렬 계산
correlation_matrix = titanic.drop('Passengerld', axis=1).corr(numeric_only=True)
...
# showmeans=False는 평균값을 표시하지 않도록 하고, showmedians=True는 중앙값을 표시하도록함
...
# x, y 설정
x = parch_counts.index.astype(str) #데이터 프레임의 인덱스를 문자열로 반환
...
#첫 번째 그래프 설정 (선 그래프)
ax1.plot(x, y1, '-s', color='indigo',  markersize=7, linewidth=5, alpha=0.7, label='Not Survived')label='Not Survived')
